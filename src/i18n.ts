...
const issueMessagePatternsEn: PatternTranslator[] = [
  ...
  { pattern: /^图中存在环路。请移除循环连接。$/, render: () => "A cycle was detected in the graph. Remove the looped connections." },
  ...
  { pattern: /^TransformerEncoder 需要 3D token 序列。$/, render: () => "TransformerEncoder expects a 3D token sequence." },
  ...
  { pattern: /^Graph 存在环，无法完成 shape 推导。$/, render: () => "Shape inference cannot run because the graph contains a cycle." },
